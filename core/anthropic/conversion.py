...
def _convert_image_block(block: Any) -> dict[str, Any]:
    """Convert Anthropic image block to OpenAI-compatible image format."""
    source = get_block_attr(block, "source", {})
    if not isinstance(source, dict):
        return {"type": "text", "text": "[image conversion failed: invalid source]"}
    ...
    return {
        "type": "text",
        "text": "[image conversion failed: unsupported source type]",
    }
    ...
    def _convert_user_message_with_injection(
        ...
        for block in content:
            ...
            if block_type == "text":
                ...
                flush_text()
                content_items.append(_convert_image_block(block))
